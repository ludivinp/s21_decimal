...
START_TEST(div_test_08) {
  s21_decimal tmp = {{0, 0, 0, 0}};

  int ret = s21_div(num1[_i], num2[_i], &tmp);
  ck_assert_int_eq(tmp.bits[0], results[_i].bits[0]);
  ck_assert_int_eq(tmp.bits[1], results[_i].bits[1]);
  ck_assert_int_eq(tmp.bits[2], results[_i].bits[2]);
  ck_assert_int_eq(tmp.bits[3], results[_i].bits[3]);
  ck_assert_int_eq(ret, 0);
}
END_TEST

START_TEST(div_test_09) {
  s21_decimal tmp = {{0, 0, 0, 0}};
  int ret = s21_div(err_num1[_i], err_num2[_i], &tmp);
  ck_assert_int_eq(ret, err_result[_i]);
}
END_TEST

Suite *s21_div_suite() {
  TCase *tcase = tcase_create("s21_div_tcase");
  Suite *suite = suite_create("s21_div");

  tcase_add_test(tcase, div_test_01);
  tcase_add_test(tcase, div_test_02);
  tcase_add_test(tcase, div_test_03);
  tcase_add_test(tcase, div_test_04);
  tcase_add_test(tcase, div_test_05);
  tcase_add_test(tcase, div_test_06);
  tcase_add_test(tcase, div_test_07);
  tcase_add_loop_test(tcase, div_test_08, 0,
                      sizeof(num1) / sizeof(s21_decimal));
  tcase_add_loop_test(tcase, div_test_09, 0,
                      sizeof(err_num1) / sizeof(s21_decimal));
